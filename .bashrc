...
alias ll='ls -g --almost-all --classify --no-group --human-readable --group-directories-first' # long, no owner, almost-all, classified, no groups, human-readable sizes, directories first
alias la='ls -l --all        --classify            --human-readable --group-directories-first' # long,           all,        classified,            human-readable sizes, directories first
alias  l='ls -g              --classify --no-group --human-readable --group-directories-first' # long, no owner,             classified, no groups, human-readable sizes, directories first
...
__pmt_build_ps1() {
    local exit=$?
    local user="\[\033[0;32m\]${GITHUB_USER:+@$GITHUB_USER}${GITHUB_USER:-\u} "
    local dir="\[\033[1;34m\]\w "
    ...
    local dollar
    [ "$exit" -ne 0 ] \
        && dollar="\[\033[1;31m\]\$\[\033[0m\]" \
        || dollar="\[\033[0m\]\$"

    PS1="${user}${dir}${branch}${dollar} "
}
PROMPT_COMMAND='__pmt_build_ps1'
